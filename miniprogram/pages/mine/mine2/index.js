...
Page({
  ...
  onLoad: function (options) {
	...
	this.data.par.MemberID=options.con
  ...
  querydynamic(con){
    ...
    Api.querydynamic('lxy_contact/queryDynamic.action',{
      ...
      //     that.data.con[i].pl=''
      //     that.data.con[i].Comments=Number(that.data.con[i].Comments)
      //     // that.data.con[i].Birth=that.age_Conversion(that.data.con[i].Birth)
      //     // that.data.con[i].CreateTime=that.funtime(that.data.con[i].CreateTime)
      //     if(JSON.parse(ress.data).Records){
      //       for(var j=0;j<JSON.parse(ress.data).Records.length;j++){
      //         if(that.data.con[i].DynamicID==JSON.parse(ress.data).Records[j].RelationID){
      //           console.log('1212')
      //           if(that.data.con[i].pl==''){
      //             that.data.con[i].pl=JSON.parse(ress.data).Records[j].AvatarUrl
      //           }else{
      //             that.data.con[i].pl=that.data.con[i].pl+','+JSON.parse(ress.data).Records[j].AvatarUrl
      //           }

      //           if(wx.getStorageSync('MemberID')){
      //             if(JSON.parse(ress.data).Records[j].MemberID==wx.getStorageSync('MemberID')){
      //               that.data.con[i].ydz='0'
      //             }
      //           }
      ...
      //     console.log(that.data.con[i].pl)

      //     that.data.con[i].pl=that.data.con[i].pl.split(',')
      //   }
      // })
      ...
      // for(var i=0;i<that.data.con.length;i++){
      //   that.data.con[i].PicUrls=that.data.con[i].PicUrls.split(',')

      //   that.data.con[i].Likes=Number(that.data.con[i].Likes)
      // }
      ...
      if(that.data.btnactive1==2){
        Api.queryCircleLinkMemberID('lxy_contact/queryCircleLinkMemberID.action',{
          ...
          // for(var i=0;i<that.data.con.length;i++){
          //   if(that.data.con[i].LinkMemberID==wx.getStorageSync('MemberID')){
          //     con.push(that.data.con[i])
          //   }
          // }
        })

        
      }else if(that.data.btnactive1==3){
        var con=[]
        ...
        Api.queryCircleMemberID('lxy_contact/queryCircleMemberID.action',{
          ...
          if(JSON.parse(res.data).Records){
            var meaasge=JSON.parse(res.data).Records
  ...
  onShareAppMessage: function () {
    return {
      title: 'tata',
      // desc: '分享邀请，获取推荐奖励',
      path: 'pages/firstpage/index?con='+wx.getStorageSync('MemberID'),
      success: function (res) {
  ...
  but(){
    ...
    Api.addLikes('lxy_contact/addLikes.action',{
      ...
      Api.sendIMReq('lxy_contact/sendIMReq.action',{
        ...
        Message: JSON.parse(res.data).CommentID,
        MsgType: 3,
        ContentType: 5,
  ...
  lik(e){
    ...
    console.log(this.data.con3.DynamicID)

    console.log(this.data.con4)

    Api.deleteDynamic('lxy_contact/deleteDynamic.action',{
      DynamicID: this.data.con3.DynamicID,
      //LinkMemberID: this.data.con1,
      // LikeType: 2,
      BusinessID: getApp().globalData.BusinessID
    }).then(res=>{
      that.data.con.splice(that.data.con4,1)
